...
)
{
	...
	uint8_t& pui8RandomByte = ChooseRandomValueReference<uint8_t>( pui8Buffer, size );
	pui8RandomByte ^= ( 1 << RandomPosition( 0, 7 ) );
...
)
{
	...
	T& valueInBuffer = ChooseRandomValueReference<T>( pui8Buffer, size );
	valueInBuffer = static_cast<T>( spanInteresting[ RandomPosition( 0, spanInteresting.size() - 1 ) ] );
...
)
{
	...
	uint8_t& value = ChooseRandomValueReference<uint8_t>( pui8Buffer, size );
	value = RandomPosition( 1, std::numeric_limits<uint8_t>::max() );
...
)
{
	...
	else
	{
		...
		if( bRandomRepeatedByte || size == 0 )
			...
			ui8RandomByte = ChooseRandomValueReference<uint8_t>( pui8Buffer, size );
...
)
{
	...
	else
	{
		...
		uint8_t ui8RandomByte = RandomPosition( 0, 1 ) ?
				ChooseRandomValueReference<uint8_t>( pui8Buffer, size ) :
...
template<class T>
T& CAFLMutationFunctions::ChooseRandomValueReference(
	uint8_t* pui8Buffer,  //!< Buffer to choose from.
	size_t size  //!< Size of the buffer.
)
{
	return *reinterpret_cast<T*>( pui8Buffer + ChooseIndexOfRandomBlock( pui8Buffer, size, sizeof( T ) ) );
}
